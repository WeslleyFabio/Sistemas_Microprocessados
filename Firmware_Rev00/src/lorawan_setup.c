...
void LORAWAN_init(uint8_t region){
	...
	LoraStartTx( TX_ON_EVENT); //Every 15 seconds sends msg? TX_ON_TIMER or TX_ON_EVENT ?????
	//clear pending IRQs
...
static void LoraStartTx(TxEventType_t EventType){
	...
	if (EventType == TX_ON_TIMER)
	{
		...
		TimerSetValue( &TxTimer,  APP_TX_DUTYCYCLE); // APP_TX_DUTYCYCLE is 15 seconds
		//Callback call
...
static void OnTxTimerEvent( void* context ){
	...
	char LoraBuffer[] = "OnTxTimerEvent";
	//Starts timer
	TimerStart(&TxTimer);

	//Sets send flag = true

	appData.Buff = (uint8_t*)LoraBuffer;
	appData.BuffSize = strlen((const char*)appData.Buff);
	appData.Port = LORAWAN_APP_PORT;

	lorawan_send(&appData);

	memset(LoraBuffer, 0, sizeof(LoraBuffer)); //clears the string after sending

}

void CallTxEvent(uint8_t *buffer,uint8_t size) //NEW Tx FUNCTION
{
